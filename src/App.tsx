...
// Generate a random accessible five-color theme
function generateTheme(): Theme {
  // Pick a random hue for the base color
  const baseHue = Math.floor(Math.random() * 360);
  
  // Generate colors using color theory principles
  const accent = chroma.hsl(baseHue, 0.7, 0.5).hex();
  const highlight = chroma.hsl((baseHue + 60) % 360, 0.6, 0.6).hex(); // Triadic harmony
  const background = chroma.hsl(baseHue, 0.1, 0.95).hex(); // Very light, low saturation
  const card = chroma.hsl(baseHue, 0.05, 0.98).hex(); // Slightly lighter than background
  const foreground = chroma.contrast(accent, '#fff') > 4.5 ? '#1a1a1a' : '#ffffff'; // High contrast text
